...
function AnswerCheck(){
	var check = $(".answer_input:checked").val();
	if(check){
		if (check==answerList[i])
		{
			...
			if(i==maxquestions)
			{
				if (solution_ok[i-1]=='ok'){
					...
					$('.explainButtonArea').text('').append('<div class="exitPlay"><button onclick="exit_play()" class="btn btn-danger mt-2"> 빠져나가기</button></div>');
				}
			}else{
				if(solution_ok[i-1]=='ok'){
					...
					$('.explainButtonArea').text('').append('<div class="nextQuizButton" onclick="nextQuiz('+i+')"></div>');	
				}
			}	
		}
		else{
