...
(function ($) {
  ...
  $(function () {
    var myDropzone = $("#imageupload").dropzone({
      url: BASE_URL + "tutor/quiz/dropzone-file",
      maxFilesize: 5,
      maxFiles: 5,
      ...
      removedfile: function (file) {
        ...
        $.ajax({
          type: "POST",
          url: BASE_URL + "tutor/quiz/dropzone-file-remove",
          data: 
          {
            filename: name
          },
  ...
  $(document).ready(function () {
    ...
    table = $("#table").DataTable({
      ...
      order: [],
      ajax: {
        url: BASE_URL + "tutor/QuizController/quiz_list?category="+category,
        type: "POST",
      },
    ...
    question_table = $("#table_question").DataTable({
      ...
      order: [],
      ajax: {
        url: BASE_URL + "tutor/quiz/question-list/" + quiz_id,
        type: "POST",
      },
        ...
        function (isConfirm) {
          if (isConfirm == true) {
            $.ajax({
              ...
              url: BASE_URL + "tutor/quiz/delete-image/" + quiz_id,
  ...
  $(document).ready(function () {
    ...
    $(document).on("change", "#list_category_id", function (e) 
    {
      ...
      var link  = BASE_URL + "tutor/quiz/?category="+category;
      window.location.href = link;
    });
    ...
    $(document).on('change','.is_quiz_active_togle',function(e){
        ...
        $.ajax({
            url: BASE_URL+"tutor/QuizController/update_quiz_status/"+id,
            type: "POST",
