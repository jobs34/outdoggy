...
function get_date_formate($date)
{
    ...
    function show_categories($categories, $parent_id = 0, $char = '')
    {
        ...
        if ($cate_child)
        {
            if($parent_id) { echo '<ul class="dropdown-menu dropdown-menu-arrow">'; } else { echo '<ul class="dropdown-menu">'; }
            ...
            foreach ($cate_child as $key => $item)
            {
                ...
                if($item['childs']) {
                    echo '<a href="#" class="dropdown-item dropdown-list-group-item dropdown-toggle">'.$item['category_title'].'<i class="fas fa-long-arrow-alt-right float-right"></i></a>';
                }
                echo '<span style="" onclick="javascript:location.href=\''.base_url('category/'.$item['category_slug']).'\'" >All '.$item['category_title'].'</span>';
    ...
    function show_categories_simple($categories, $parent_id = 0, $char = '')
    {
        $cate_child = array();
        foreach ($categories as $key => $item)
        {
        ...
        if ($cate_child)
        {
            ...
            foreach ($cate_child as $key => $item)
            {
                ...
                if($item['childs']) {
                    echo '<a href="';
                    echo base_url('category/'.$item['category_slug']).'/"'.'class="dropdown-item">'.$item['category_title'].'</a>';
                }
                echo '</li>';
            }
            echo '</ul>';
...
if (!function_exists('get_study_material_detail_page_url_by_id'))
{
    function get_study_material_detail_page_url_by_id($tab_study_material_id)
    {
        ...
        if($tab_study_material_id)
        {
            ...
            */
            $sm_slug_url = base_url('study-material/').$tab_study_material_id;
        }
