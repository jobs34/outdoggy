      ...
      <div class="card">
         <div class="card-body">
            ...
            <div class="row">
               ...
               <div class="col-6">
                  <div class="form-group <?php echo form_error('slug') ? ' has-error' : ''; ?>">
                     <?php echo form_label(lang('admin_slug'), 'slug'); ?>
                     <span class="required">*</span>
                     <?php $populateData = (!empty($this->input->post('slug')) ? $this->input->post('slug') : (!empty($editData['post_slug']) && isset($editData['post_slug']) ? $editData['post_slug'] : '' )); ?>
                     <input type="text" name="slug" id="slug" class="form-control" value="<?php echo xss_clean($populateData);?>">
                     <span class="small form-error"> <?php echo strip_tags(form_error('slug')); ?> </span>  
                  </div>
               </div>
               <div class="col-6">
                  <div class="form-group">
                     연관 정보
                     <?php $populateData = (!empty($editData['rel_study']) && isset($editData['rel_study']) ? $editData['rel_study'] : (!empty($this->input->post('relstudy')) ? $this->input->post('relstudy') : '' )); ?>
                     <select class="form-control" name="relstudy">
                        <option value=""><?php echo lang('admin_select_one'); ?></option>
                        <?php
                        foreach($license_category as $license_key=>$license_value)
                        {
                           $selected = ($license_value->id == $populateData) ? "selected" : '';
                           ?>
                           <option <?php echo xss_clean($selected);?> value="<?php echo xss_clean($license_value->id);?>"><?php echo xss_clean($license_value->category_title);?>   </option>
                        <?php } ?>
                     </select>
                  </div>
               </div>               
            ...
            <div class="form-group">
               ...
               <textarea name="description" id="description" rows="5" class="form-control editor"><?php echo $populateData;?></textarea>
            </div>
            <div class="form-group">
