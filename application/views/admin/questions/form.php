  ...
  <div class="col-12 col-md-12 col-lg-12">
    ...
    <div class="col-12">      
        ...
        <div class="row"> 
          ...
          <div class="col-12">
            <div class="form-group <?php echo form_error('title') ? ' has-error' : ''; ?>">
              ...
              ?>
              <textarea name="title" id="title" class="form-control custom-area" rows="4" ><?php echo xss_clean($populateData);?></textarea>
          ...
          <div class="col-6">
            <div class="question_img_block form-group <?php echo form_error('image_upload_error') ? ' has-error' : ''; ?>">
                <?php echo  form_label(lang('문제 첨부 파일'), 'image'); ?>
                <?php 
          ...
          <div class="col-12">
            <div class="form-group <?php echo form_error('solution') ? ' has-error' : ''; ?>">
              <?php echo  form_label('해설', 'solution'); ?>
              ...
              ?>
              <textarea name="solution" id="p_desc" class="form-control custom-area" rows="3" ><?php echo xss_clean($populateData);?></textarea>
          ...
          <div class="col-2">
            <div class="form-group">
              <?php echo form_label('해설 덧붙이기', 'select'); ?>
              <?php 
              ...
              <select name="addon_type" id="addon_type" class="form-control">
                  ...
                  <option value="1" <?php if($populateData==1) echo "selected" ?> >클래스 코드</option>                  
                  <option value="2" <?php if($populateData==2) echo "selected" ?> >유튜브 임베드 코드</option>
                  <option value="3" <?php if($populateData==3) echo "selected" ?> >Vimeo 임베드 코드</option>
                  <option value="4" <?php if($populateData==4) echo "selected" ?> > 외부 영상 URL</option>
                  <option value="5" <?php if($populateData==5) echo "selected" ?> > 기타 URL</option>
              </select>
            </div>
          </div>
          ...
          <div class="col-6">
            <div class="form-group">
              <?php echo form_label('해설 덧붙이기 상세코드', 'text'); ?>
              <?php 
        ...
        <div class="question_img_block form-group <?php echo form_error('solutionimage_error') ? ' has-error' : ''; ?>">
              <?php echo  form_label('해설 첨부 이미지', 'solutionimage'); ?>
              <?php 
            ...
            <div class="after_ticket_section row" <?php echo $display_none ; ?>>
              ...
              <div class="col-md-2">
                <div class="form-group mt-4 pt-2">
                  <button class="btn btn-primary  btn-block add-more" data-index="<?php echo $no_of_option; ?>" type="button">
...
<?php if($update)
    {   
        //----------------------------------------------
        if(empty($nextques_data)){
            $next_no = 0;
            $next_title = "다음 문제가 없습니다";
       }else {
            $next_no = $nextques_data->id;
            $next_title = $nextques_data->title;
       }
        if(empty($prevques_data)){
            $prev_no = 0;
            $prev_title = "이전 문제가 없습니다";
       }else {
            $prev_no = $prevques_data->id;
            $prev_title = $prevques_data->title;
        }       
        //---------------------------------------------
        ?>
            <div class="col-md-12">
                <a href="<?php echo base_url('admin/questions/update/').$quiz_id."/".$next_no;?>" class="btn btn-danger px-2">다음 문제 바로 가기</a><span class="px-3"><?php echo $next_title;?></span>
            </div>
        <hr>
                    <div class="col-md-12">
                <a href="<?php echo base_url('admin/questions/update/').$quiz_id."/".$prev_no;?>" class="btn btn-info px-2">이전 문제 바로 가기</a><span class="px-3"><?php echo $prev_title;?></span>
            </div>
            <hr>
          <div class="clearfix"></div>
<?php
