...
class StudyModel extends CI_Model {
   ...
   public function __construct() {
    ...
    function get_study_section_info($study_section_id)
    {
        return $this->db->select('study_section.*')->where('id',$study_section_id)->get('study_section')->row();
    }
    ...
    function get_study_material_and_file_by_id($study_material_id)
    {
        ...
        return $this->db->select('study_material.*,
            (select category_title from category where category.id = study_material.category_id ) as category_title,
    ...
    function get_study_material_and_file_by_id_bk($study_material_id)
    {
        $user_id = isset($this->user['id']) ? $this->user['id'] : 0;
        return $this->db->select('study_material.*,
            (select category_title from category where category.id = study_material.category_id ) as category_title,
    ...
    function get_study_material_previewcontent($study_material_id)
    {
        $this->db->where('study_material_id',$study_material_id);
        return $this->db->order_by('section_id','asc')->order_by('material_order','asc')->get('study_material_content')->row();
    }
