...
class UsersModel extends CI_Model {
    ...
    function add_user($data = array())
    {
        if ($data) {
            ...
            $cel_num = (isset($data['cel_num']) && $data['cel_num']) ? $data['cel_num'] : 0; //subadmin
            $etc_info = (isset($data['etc_info']) && $data['etc_info']) ? $data['etc_info'] : 0; //subadmin
            ...
            " . $this->db->escape($role) . ",
            " . $this->db->escape($cel_num) . ",
            " . $this->db->escape($etc_info) . ",
    ...
    function edit_user($data = array()) {
        ...
        if ($data) {
            ...
            $cel_num = (isset($data['cel_num']) && $data['cel_num']) ?  $data['cel_num'] : 0;
            ...
            cel_num = " . $this->db->escape($cel_num) . ",
            etc_info = " . $this->db->escape($etc_info) . ",
    ...
    function edit_profile($data = array(), $user_id = NULL) {
        ...
        if ($data && $user_id) {
          ...
          //  $profileimg = $file['file_name'];
            ...
            // $status = $data['status']=='Active' ? 1 : 0;
            $cel_num = isset($data['cel_num']) ? $data['cel_num'] : 0;
            $etc_info = (isset($data['etc_info']) && $data['etc_info']) ? $data['etc_info'] : 0;
    ...
    {
        return $this->db->select('email,txn_id,item_name,item_price,payment_status,created,payment_gateway,token_no,item_price_currency,invoice_no,(select first_name from users where id = payments.user_id)as first_name,(select last_name from users where id = payments.user_id)as last_name,(select description from quizes where id = payments.item_id)as description')
