...
class BlogModel extends CI_Model {
    ...
    function category_delete($id) {
        ...
        $this->db->where('blog_category_id', $id)->delete('blog_post');
    }

    function alluser()
    {
    	return $this->db->select('id,username,first_name,image as avatar,last_name')->get('users')->result();
    }
    ...
    function get_blog_total_record($category_id)
    {
        ...
        return $this->db->where('post_status',1)
        ->get('blog_post')->result();
    }
    ...
    function get_blog_post_per_page($category_id, $pro_per_page, $page)
    {
        ...
        $this->db->select('blog_post.*,
            (select title from blog_category where blog_category.id = blog_post.blog_category_id) as title,
            (select category_title from category where id = blog_post.rel_study) as rel_name');
    ...
    function get_blog_post_per_page_bk($category_id, $pro_per_page, $page)
    {
        $user_id = isset($this->user['id']) ? $this->user['id'] : 0;
        $this->db->select('blog_post.*,
            (select title from blog_category where blog_category.id = blog_post.blog_category_id) as title,
            (select slug from blog_category where blog_category.id = blog_post.blog_category_id) as blog_category_slug,
            (select first_name from users where users.id = blog_post.post_author_id) as first_name,
            (select image from users where users.id = blog_post.post_author_id) as avatar,
            (select count(id) from post_count where post_id = blog_post.id) as view_total_post,
            (select count(id) from post_like where post_id = blog_post.id AND user_id = '.$user_id.') as is_like,
    ...
    function get_post_by_slug($post_slug)
    {
        ...
        return $this->db->select('blog_post.*,
            (select title from blog_category where blog_category.id = blog_post.blog_category_id) as title,          
            (select slug from blog_category where blog_category.id = blog_post.blog_category_id) as category_slug,
            (select first_name from users where users.id = blog_post.post_author_id) as first_name,
            (select image from users where users.id = blog_post.post_author_id) as avatar,
            (select id from post_like where post_like.post_id = blog_post.id AND post_like.user_id = '.$user_id.') as is_like,
            (select count(id) from post_like where post_like.post_id = blog_post.id) as total_like
            ')
        ->where('post_slug',$post_slug)->where('post_status',1)->get('blog_post')->row();
    }

    function all_blog_category()
    {
        return $this->db->select('id,title,slug')->order_by('id', "desc")->get('blog_category')->result();
    }

    function all_relstudy_category() {
        return $this->db->where('category_is_delete',0)->where('category_status',1)->where('parent_category >',0)->order_by('category_title','asc')->get('category')->result();
    }

    function relstudy_ctp01_category() {
    ...
    {
        return $this->db->select('id,title,slug,description')->where('slug',$category_slug)->get('blog_category')->row();
    }
    ...
    {
        return $this->db->select('blog_post.*,(select title from blog_category where blog_category.id = blog_post.blog_category_id) as title,(select first_name from users where users.id = blog_post.post_author_id) as first_name,(select image from users where users.id = blog_post.post_author_id) as avatar')
    ...
    function get_relstudy_post($rel_study,$post_id)
    {
        return $this->db->select('id, post_title, post_slug, post_image, (select title from blog_category where blog_category.id = blog_post.blog_category_id) as category_title')
            ->where('rel_study',$rel_study)
            ->where('id !=', $post_id)
            ->where('post_status',1)
            ->limit(10)
            ->order_by('id', "desc")
    ...
    function get_relcategory_post($blog_category_id	,$post_id)
    {
        return $this->db->select('id, post_title, post_slug, post_image')
    ...
    function get_rel_study_by_id($rel_study) {
        return $this->db->select('id, category_title, category_slug, category_image, gubun, category_description, category_ext')->where('id', $rel_study)->where('category_status',1)->where('category_is_delete',0)->order_by('category.order','asc')->get('category')->row();
    }
