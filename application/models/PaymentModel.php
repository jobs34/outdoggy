...
class PaymentModel extends CI_Model  
{
    ...
    function get_user_payment_history($user_id,$purchases_type)
    {
    ...
    function get_payment_simpledata($id)
    {  
        return $this->db->select('id, user_id, item_id, purchases_type')
            ->where('id', $id)->get('payments')->row();
    }
    
    function get_payment_detail_by_id($id)
    {  
        return $this->db->select('id,item_id as relation_id, purchases_type,email,payer_name,item_name,item_price,payment_status,created,modified,payment_gateway,token_no,item_price_currency,customer_id,invoice_no,(select first_name from users where id = payments.user_id)as first_name')
            ->where('id', $id)->get('payments')->row();
    }
    
    function get_last_enroll_status($item_id)
    {
        $user_id = isset($this->user['id']) ? $this->user['id'] : 0;
        return $this->db->where('study_material_id',$item_id)
                        ->where('user_id',$user_id)
                        ->get('study_material_user_histry')
                        ->row();
    }        
    
    //--- 아래의 Fuction 대신  get_payment_pending_status 사용
    function get_payment_status_old($quiz_id,$user_id,$purchases_type)
    {
        $status = '(payment_status="pending" or payment_status="fail")';
        $this->db->select('id,token_no,payment_status');
        $this->db->where('item_id',$quiz_id);
    ...
    function get_payment_pending_status($item_id,$user_id,$purchases_type)
    {
        //원래는 Fail을 포함했으나 Pending 만 체크해서 신청 전에 처리하는 것으로 변경
        $status = '(payment_gateway="bank" and payment_status="pending")';
        $this->db->select('id,created,payment_status');
    ...
    function get_last_payment_status($purchases_type,$item_id)
    {
        $user_id = isset($this->user['id']) ? $this->user['id'] : 0;
    ...
    function get_user_paid_study_material_obj($user_id)
    {
    ...
    function update_card_status($payment_id,$user_id,$data)
    {
        $transaction_data = array("customer_id"=>$data['customer_id'],"modified"=>$data['modified'],"payment_status"=>$data['status']);
    ...
    function delete_payment_detail($id,$token_no,$user_id) {
        $this->db->where('id', $id);
        $this->db->where('user_id', $user_id);
        $this->db->where('token_no', $token_no)->delete('payments');
    }
    ...
    {
        $is_paid_quiz_obj = $this->db->select('quizes.id,quizes.price,quizes.title,quizes.category_id,quizes.user_id, (select payments.id from payments where payments.item_id =  quizes.id AND  payments.purchases_type = "quiz" AND payments.payment_status = "succeeded" AND payments.user_id = '.$user_id.' limit 1) as quiz_payment_id')
            ->where('is_quiz_active',1)
            ->get('quizes')->result();
    ...
    function get_membership_by_payment_id($id,$payment_gateway,$purchases_type)
    {
        return $this->db->select('payments.purchases_type,quiz_id,user_id,payment_gateway,
                    (select membership.id from membership where membership.id = payments.item_id)as membership_id,
                    (select membership.duration from membership where membership.id = payments.item_id)as duration')
