...
class TestModel extends CI_Model 
{
    ...
    function get_question_by_quiz_id($quiz_id,$number_questions) 
    {
        return $this->db->select('questions.*,
            ...
            (SELECT content FROM paragraph where paragraph.id = questions.question_paragraph_id) as question_paragraph_text,(SELECT title FROM section where section.id = questions.question_section_id) as question_section_name,
...
function site_leader_board_quiz_history()
{
    $return = $this->db->select('SUM(earned_points) as total_earned,participants.*,users.id as user_id, first_name, last_name, 
        ...
        ->order_by('take_time_for_exam','ASC')
        ->order_by('age','DESC')
