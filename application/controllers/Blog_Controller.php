...
class Blog_Controller extends Public_Controller {
    ...
    function list($category_slug=NULL, $page_no=NULL)
    {
        ...
        $title = $category_id ?  $blog_category_data->title : "매거진";
        $meta_title_tmp = "캠핑 2막 매거진 - 초보에게 도움되는 캠핑 정보";
        $meta_description_tmp = "초보 캠퍼들에게 도움이 되는 정보 모음입니다.";
        $page_title = $title;
    	$this->set_title($meta_title_tmp);

        $meta_data = array('meta_title' => $meta_title_tmp,'meta_description' =>  $meta_description_tmp);

        $content_data = array('page_title' => $page_title,'category_slug' => $category_slug,'pagination' => $page_links, 'blog_post_data' => $blog_post_data, 'blog_category_data' => $blog_category_data,);
    ...
    function detail($post_slug = NULL)
    {
        ...
        if (isset($rel_study) && !empty($rel_study)){
            ...
            $nav_slug = $lecture_category->category_slug;
            $adproduct_data = $this->AdProductModel->get_category_product($rel_study);
        }  
        $new_post = $this->BlogModel->get_relcategory_post($post_detail_data->blog_category_id, $post_detail_data->id);
        $this->set_title($page_title);
        $meta_data = array('meta_title' => $post_detail_data->meta_title, 'meta_keywords' => $post_detail_data->meta_keywords, 'meta_description' =>  $post_detail_data->meta_description,'title' => $post_detail_data->post_title,'description' => $post_detail_data->post_description,'image' => $post_detail_data->post_image);

        $content_data = array('Page_message' => $page_title, 'page_title' => $page_title, 'nav_slug' => $nav_slug, 'post_detail_data' => $post_detail_data,'related_post' => $related_post, 'new_post' => $new_post,'lecture_category' => $lecture_category, 'adproduct_data' => $adproduct_data);
