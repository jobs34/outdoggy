...
class Profile extends Private_Controller {
    ...
    { 
        ...
        $this->form_validation->set_rules('password', '비밀번호', 'min_length[5]|matches[password_repeat]');
        $this->form_validation->set_rules('password_repeat', '비밀번호 재입력', 'min_length[5]');
        $this->form_validation->set_rules('email', '이메일', 'required|trim|max_length[128]|valid_email|callback__check_email');
        ...
        $paid_quizes_array = $this->PaymentModel->get_user_paid_quiz_obj($user_id);
        ...
        */
        ...
        $data['content'] = $this->load->view('my_profile', $content_data, TRUE);        
    ...
    function _check_phone_number($cel_num) {
            ...
            {
                $this->form_validation->set_message('_check_phone_number', "10자리 이상의 숫자만 가능합니다.");
                return FALSE;
            }
        }    
    ...
    function view_payment_detail()
    {
       ...
       // $payment_data = $this->PaymentModel->get_payment_detail_by_id($id);
        $payment_data = $this->PaymentModel->get_payment_detail_by_id('21');
