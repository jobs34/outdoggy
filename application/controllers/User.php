...
class User extends Public_Controller {
    ...
    function login() 
    {
        ...
        else
        {
            ...
            else 
            {
                ...
                {
                    // 로그인 후 랜딩 페이지 설정 (나중에 하자)
                    //redirect(base_url());
                    //아래 쪽은 테스트
                    //$item_id = "3";
                    $rtnurl = $this->input->get('returnUrl');
                    redirect(base_url("$rtnurl"));
                    //$rtnurl = $this->input->get('returnUrl');
                    //redirect('aaa='.$rtnurl);
                }
            }
    ...
    function register($is_tutor = 'user') 
    {
        ...
        // validators
        $this->form_validation->set_error_delimiters($this->config->item('error_delimeter_left'), $this->config->item('error_delimeter_right'));
        $this->form_validation->set_rules('username', '아이디', 'required|trim|min_length[5]|max_length[30]|callback__check_username');
        $this->form_validation->set_rules('first_name', '이름', 'required|trim|min_length[2]|max_length[32]');
        $this->form_validation->set_rules('password', '비밀번호', 'required|trim|min_length[5]');
        $this->form_validation->set_rules('password_repeat', '비밀번호 재입력', 'required|trim|matches[password]');
        $this->form_validation->set_rules('email', '이메일', 'required|trim|max_length[256]|valid_email|callback__check_email');
        $this->form_validation->set_rules('cel_num', '휴대전화', 'required|trim|max_length[15]|callback__check_phone_number');
        ...
        if ($this->form_validation->run() == TRUE) 
        {
            ...
            if($user_register_response && is_array($user_register_response))
            {
                ...
                $this->session->language = 'Korean';
                // build the validation URL
                $encrypted_email = sha1($this->input->post('email', TRUE));
                $user_full_name = $this->input->post('first_name', TRUE);
                ...
                $email_msg = "안녕하세요. $user_full_name 님. 회원가입이 완료되었습니다.";
                $mail_subject = "회원가입 완료 메일 : ".$this->settings->site_name;
                ...
                if($new_user_data->role == "tutor")
                {
                    $this->sendmail->sendTo($mail_to, $mail_subject, $recipet_name, $email_msg);
        ...
        $data = $this->includes;
        // set content data
        $content_data = array('cancel_url' => base_url(),'is_tutor' => $is_tutor, 'user' => NULL, 'password_required' => TRUE,'customfields'=>$customfields);
        // load views
    ...
    function _check_username($username) {
        ...
        else 
        {
            ...
            {
                $this->form_validation->set_message('_check_username', "최소 5자리 이상의 영문자 혹은 숫자만 가능합니다.");
                return FALSE;
            }
    ...
    function _check_phone_number($cel_num) {
            if(preg_match('/^[0-9]{10,}$/', $cel_num))
            { 
                // for numbers only
                return $cel_num;
            }
            ...
            {
                $this->form_validation->set_message('_check_phone_number', "숫자만 가능합니다.");
                return FALSE;
            }
    }
