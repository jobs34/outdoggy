...
class Excel_import extends Admin_Controller {
    ...
    function index($post_quiz_id = NULL) 
    {
        ...
        $content_data = array('quiz_name_array' => $quiz_name_array,'post_quiz_id'=>$post_quiz_id,'quiz_all_data'=>$quiz_data);
        // load views
    ...
    function study_data_import($post_s_m_id = NULL) 
    {
        ...
        else
        {
            ...
            if (!$this->upload->do_upload('excel_file')) 
            {
                ...
                $this->session->set_flashdata('error', $error);
                return redirect(base_url('admin/study-data/import/'.$post_s_m_id));
            }
            else
            {
        ...
        // load views
        $data['content'] = $this->load->view('admin/import/study_data/form', $content_data, TRUE);
        $this->load->view($this->template, $data);
    ...
    private function import_study_material_Excel_data($file_name, $s_m_id) 
    {
        ...
        catch (Exception $e)
        {
            $error =  lang('unable_to_read_this_file_formate');
            $this->session->set_flashdata('error', $error);
            return redirect(base_url('admin/study-data/import/'.$s_m_id));        
        }
        ...
        {   
            $section_name = isset($study_data_contant_array['SECTION-NAME']) ? trim($study_data_contant_array['SECTION-NAME']) : NULL;
    ...
    function extcourse_data_import($ext_course_id = NULL) 
    {
        $ext_course_data = $this->ExtModel->get_ext_course_by_id($ext_course_id);
        $this->form_validation->set_rules('ext_course_id', lang('admin_excel_quiz_name'), 'required|numeric|trim');
        ...
        if ($this->form_validation->run() == false OR empty($this->input->post('ext_course_id'))) 
        {
            $this->form_validation->error_array();
        }
        else
        {
            ...
            if (!$this->upload->do_upload('excel_file')) 
            {
                ...
                $this->session->set_flashdata('error', $error);
                return redirect(base_url('admin/extcourse/import/'.$ext_course_id));
            }
            else
            {
            ...
            $ext_course_id = $this->input->post('ext_course_id',TRUE); 

            $import = $this->import_ext_course_Excel_data($file['file_name'], $ext_course_id);
        ...
        $extcourse_name_array = array();
        $all_extcourse = $this->ImportModel->get_all_ext_course();
        foreach ($all_extcourse as $extcourse_array) 
        {
            $extcourse_name_array[''] = 'Select Ext';
            $extcourse_name_array[$extcourse_array->id] = $extcourse_array->title;
        }

        $this->set_title("캠핑장 정보 엑셀 : ".$ext_course_data->title);
        $data = $this->includes;
        
        $content_data = array('extcourse_name_array' => $extcourse_name_array,'ext_course_id'=>$ext_course_id,'ext_course_data'=>$ext_course_data);
        // load views
        $data['content'] = $this->load->view('admin/import/extcourse_form', $content_data, TRUE);
        $this->load->view($this->template, $data);

    }


    private function import_ext_course_Excel_data($file_name, $ext_course_id) 
    {
        ...
        try
        {
           $lesson_array = (new FastExcel)->import($file_dir);
        }
        catch (Exception $e)
        {
            $error =  lang('unable_to_read_this_file_formate');
        ...
        $lesson_array = $lesson_array ? $lesson_array : array();
        $lesson_array = json_decode(json_encode($lesson_array), true);

        $lesson_content_array = array();

        $i = 0;
        $insert_count = 0;
        $skip_count = 0;
        foreach ($lesson_array as $product_detail_data) 
        {   
            $i++;
            if(empty($product_detail_data['title']))
            {
            ...
            if($i === 1)
            {
                if(empty($product_detail_data['order']) OR empty($product_detail_data['value']))
                {
                    $this->session->set_flashdata('error', lang('invalid_file_formate'));
                    return redirect(base_url('admin/extcourse/import/'.$ext_course_id));
                }
            ...
            */
            ...
            if($product_detail_data['title'])
            {
                $insert_count++;
                $lesson_content['ext_course_id'] = $ext_course_id;
                $lesson_content['material_order'] = $product_detail_data['order'];
                $lesson_content['title'] = $product_detail_data['title'];
                $lesson_content['value'] = $product_detail_data['url'];
                $lesson_content['added'] =  date('Y-m-d H:i:s');
                $lesson_content_array[] = $lesson_content;
            }
            else
            {
                $skip_count++;
        ...
        $status = $this->ImportModel->insert_bulk_ext_course_lesson($lesson_content_array);
        $respone['insert_count']   = $insert_count;     
        $respone['skip_count']   = $skip_count;  
        return $respone;
    }
