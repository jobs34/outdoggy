...
class Blog extends Admin_Controller {
    ...
    function blog_category_form($blog_category_id = null) {
        ...
        if ($this->form_validation->run() != false)
        {
            ...
            $content['slug'] = $this->input->post('slug', TRUE);
            //$content['slug'] = "as_" . date('s') . date('y') . date('m') . date('B');
            $content['parent_category'] = $this->input->post('parentcat', TRUE);
            $content['description'] = $this->input->post('description', FALSE);
    ...
    function blog_category_list() {
        ...
        foreach ($list as $blog_category) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = $blog_category->title;
            $row[] = $blog_category->slug;
    ...
    function blog_post_form($post_id = NULL)
    {
        ...
        $license_category = $this->BlogModel->relstudy_ctp01_category();
        ...
        if ($this->form_validation->run() != false)
        {
            ...
            $content['post_slug'] = $this->input->post('slug', TRUE);
            $content['rel_study'] = $this->input->post('relstudy', TRUE);
            $content['post_status'] = $this->input->post('status', TRUE);
            $content['post_description'] = $this->input->post('description', FALSE);
        ...
        $data = $this->includes;
        $content_data = array('author' => $author,'editData' => $editData, 'blog_category' => $blog_category, 'license_category' => $license_category,);
        // load views
    ...
    function blog_post_list() {
        ...
        foreach ($list as $blog_post) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = $blog_post->post_title;
            $row[] = $blog_post->post_slug;
