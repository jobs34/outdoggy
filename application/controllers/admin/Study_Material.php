...
class Study_Material extends Admin_Controller {
    function __construct() 
    {
        ...
        define('THIS_URL', base_url('admin/study'));
    ...
    function index()
    {
       ...
       $this->set_title("캠핑스쿨");
        ...
        $content_data = array();
        // load views
        $data['content'] = $this->load->view('admin/study/material_list', $content_data, TRUE);
    ...
    { 
        ...
        $category_array = $this->CategoryModel->get_ctp03_category();
        ...
        else
        {
            ...
            $study_material_content['user_id'] = $this->input->post('user_id',TRUE);
            ...
            $study_material_content['description'] = $this->input->post('description',FALSE);
        ...
        $content_data = array('category_data'=>$category_data,'all_user_data'=>$all_user_data,'study_material_id'=>$study_material_id,);
        // load views
        $data['content'] = $this->load->view('admin/study/studymaterial_form', $content_data, TRUE);
        $this->load->view($this->template, $data);
    }
    ...
    function study_material_update($study_material_id = NULL)
    {
        ...
        $study_material_data = $this->StudyModel->get_study_material_by_id($study_material_id);
        ...
        $content_data = array('category_data'=>$category_data,'all_user_data'=>$all_user_data,'study_material_id'=>$study_material_id,'study_material_data'=>$study_material_data,);
        // load views
        $data['content'] = $this->load->view('admin/study/studymaterial_form', $content_data, TRUE);
        $this->load->view($this->template, $data);
    }
    ...
    {
        $data = array();
        $list = $this->StudyModel->get_study_material();
        ...
        foreach ($list as $study_material) {
            ...
            $button = '<a href="' . base_url("admin/study/update/". $study_material->id) . '" data-toggle="tooltip"  title="'.lang("admin_edit_record").'" class="btn btn-primary btn-action mr-1"><i class="fas fa-pencil-alt"></i></a>';

            $button.= '<a href="' . base_url("admin/study/copy/" . $study_material->id) . '" data-toggle="tooltip"  title="'.lang("admin_copy_record").'" class="common_copy_record btn btn-warning mr-1"><i class="far fa-copy"></i></a>';
            ...
            $button.= '<a href="' . base_url("admin/study/delete-study-material/" . $study_material->id) . '" data-toggle="tooltip"  title="'.lang("admin_delete_record").'" class="btn btn-danger btn-action mr-1 common_delete"><i class="fas fa-trash"></i></a>';


            $row[] = $button;
            $data[] = $row;
        }
        $output = array("draw" => $_POST['draw'], "recordsTotal" => $this->StudyModel->count_all(), "recordsFiltered" => $this->StudyModel->count_filtered(), "data" => $data,);
    ...
    {
        action_not_permitted();
        $status = $this->StudyModel->delete_study_material($study_material_id); 
    ...
    function section($study_material_id = FALSE)
    {
        ...
        if($this->form_validation->run() == false)  
        {
            ...
            redirect(base_url('admin/study/material-file/'.$study_material_id));
        }
        else
        {
    ...
    function section_update($study_section_id)
    {
        ...
        else
        {
            ...
            redirect(base_url('admin/study/material-file/'.$study_section_data->study_material_id));
        }
    }
    ...
    function section_delete($study_section_id)
    {
        ...
        {
            $this->session->set_flashdata('error', lang('something_went_wrong')); 
            return redirect(base_url('admin/study'));
        }
    ...
    function material_file($study_material_id = NULL)
    {
        ...
        $this->add_css_theme('sweetalert.css')->add_js_theme('sweetalert-dev.js')->add_js_theme('bootstrap-notify.min.js')->set_title(lang('admin_study_material_file_list'));

        $data = $this->includes;
        $content_data['study_material_id'] =$study_material_id;
        $content_data['study_section_data'] =$study_section_data;
        // load views
        $data['content'] = $this->load->view('admin/study/material_file_list', $content_data, TRUE);
        $this->load->view($this->template, $data);       
    }
    ...
    function add_material_file($study_material_id = NULL,$contant_type)
    {
        ...
        if(empty($study_material_section_data))
        {
            ...
            redirect(base_url("admin/study/material-file/".$study_material_data->id));
        }
        ...
        if($this->input->post('title'))
        {
            if($contant_type == "image" OR $contant_type == "audio" OR $contant_type == "video" OR $contant_type == "pdf" OR $contant_type == "doc" )
            {
                ...
                {   
                    ...
                    {
                        $allowed_max_upload_size = 50000000;
                    ...
                    $path = "./assets/uploads/study_material";

                    $material_file_upload = $this->do_upload_file('file',$path,$allowed_formate,$allowed_max_upload_size);
                    ...
                    {
                        $this->session->set_flashdata('error', $material_file_upload['error']); 
                        $this->form_validation->set_rules('file_upload', 'Matiral File', 'required');
                    }
                    
                }
                else
                {
                    $this->form_validation->set_message('file_upload', 'Please upload File First.');
                    $this->form_validation->set_rules('file_upload', 'Matiral File ', 'required');
                }
            ...
            {
                $embed_code_contant = $this->input->post('embed_code_contant',FALSE);
                $section_id = $this->input->post('section_id',TRUE);
                $duration = $this->input->post('duration',TRUE);
                
                if(empty($embed_code_contant) && empty($upload_file_name))
                {
                    $this->session->set_flashdata('error', lang('something_went_wrong')); 
                    redirect(base_url('admin/study/material-file/'.$study_material_id));
                }
                ...
                $study_material_content['plus_contents'] = $this->input->post('plus_code_contant',FALSE);
        ...
        // load views
        $data['content'] = $this->load->view('admin/study/material_file_form', $content_data, TRUE);
        $this->load->view($this->template, $data);        
    }
    ...
    function update_study_material_content($study_material_content_id)
    {
        ...
        if($this->input->post('title'))
        {
            ...
            else
            {
                ...
                if(empty($embed_code_contant) && empty($upload_file_name) && empty($last_contant_value))
                {
                    $this->session->set_flashdata('error', lang('something_went_wrong')); 
                    redirect(base_url('admin/study/material-file/'.$study_material_id));
                }
                ...
                if($contant_type == "image" OR $contant_type == "audio" OR $contant_type == "video" OR $contant_type == "pdf" OR $contant_type == "doc" )
                {
                   if($upload_file_name)
                    {
                       $value = $upload_file_name;
                       $last_file_link = $path.'/'.$last_contant_value;
                        if($last_contant_value && file_exists($last_file_link))
                        {
                            unlink($last_file_link);
                        }
                    }
                    else
                    {
                        $value = $last_contant_value;
                    } 
                }
                ...
                $study_material_content['size'] = $file_size;
                $study_material_content['duration'] = $duration;
                $study_material_content['example'] = $example;
    ...
    function material_file_list($study_material_id = NULL)
    {
        ...
        foreach ($list as $study_material_content) {
            ...
            $button = '<a href="' . base_url("admin/study/update-material-content/" . $study_material_id ."/". $study_material_content->id) . '" data-toggle="tooltip"  title="'.lang("admin_edit_record").'" class="btn btn-primary btn-action mr-1"><i class="fas fa-pencil-alt"></i></a>';            

            $button.= '<a href="' . base_url("admin/study/delete-material-content/" . $study_material_id ."/". $study_material_content->id) . '" data-toggle="tooltip"  title="'.lang("admin_delete_record").'" class="btn btn-danger btn-action mr-1 common_delete"><i class="fas fa-trash"></i></a>';
    ...
    function delete_study_material_content($study_material_content_id)
    {
        ...
        redirect(base_url('admin/study/material-file/'.$study_content_data->study_material_id));
    }
