...
class Institution extends Admin_Controller {
    ...
    function add() 
    {
        ...
        $this->form_validation->set_rules('address', '멋진 한줄 요약', 'required|trim');
        ...
        $this->set_title("캠핑장 테마 추가");
        $data = $this->includes;

        $content_data = array('all_courses' => $all_courses);
        // load views
...
/*
        ...
        {
            $this->form_validation->set_rules('logo_error','이미지', 'required|trim');
        }
...
*/        
        ...
        $this->set_title("캠핑장 테마 업데이트");
        $data = $this->includes;

        $content_data = array('institution_id' => $institution_id, 'institution_data' => $institution_data);
        // load views
        $data['content'] = $this->load->view('admin/institution/form', $content_data, TRUE);
        $this->load->view($this->template, $data);
    }

    function update_bk($institution_id = NULL) 
    {
        if(empty($institution_id))
        {
           $this->session->set_flashdata('error', lang('invalid_url')); 
