...
class Users extends Admin_Controller {
    ...
    function tutor_list() 
    {
        ...
        foreach ($list as $tutor_data) 
        {
            ...
            $status = "<a data-user_id='$tutor_data->id' href='$url' class='btn btn-warning rounded tutor_status' id='tutor_status'>Approved</a>";

            $row[] = $status;
            $button = '<a href="' . base_url("admin/users/edit/". $tutor_data->id) . '" data-toggle="tooltip"  title="'.lang('update').'" class="btn btn-primary btn-action mr-1"><i class="fas fa-pencil-alt"></i></a>';
            $row[] = $button;
            $data[] = $row;
        }
    ...
    function add() 
    {
        ...
        $this->form_validation->set_rules('cel_num', 'cel_num', 'required|trim');
        ...
        if (!is_null($id) OR !is_numeric($id)) 
        {
            ...
            if ($user) {
                ...
                if($delete) 
                {
                    ...
                    if($user['role'] = 'tutor' OR $user['is_admin'] = 1)
                    {
                        $this->assign_admin_or_tutor_related_data_to_other($id,$assign_to);
                        // $deleted_array = array();
                        // $deleted_array['username'] = "deleted_".$user['id']."_".$user['username'];
                        // $deleted_array['email'] = "deleted_".$user['id']."_".$user['email'];
                        // $this->db->where('id',$id)->update("users",$deleted_array);
                    }
    ...
    function summary($participant_id = NULL)
    {
        ...
        $user_question_data = $this->UsersModel->get_user_question_by_participant_id($participant_id);
        ...
        $data['content'] = $this->load->view('admin/users/summary', $content_data, TRUE);
        $this->load->view($this->template, $data);      
    }
