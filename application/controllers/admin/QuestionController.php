...
class QuestionController extends Admin_Controller {
    ...
    function add($quiz_id = false) 
    {
        ...
        {   
            $question_path = "./assets/images/questions";
            $question_allowed_format = 'gif|jpg|png|bmp|jpeg';

            $upload_type = $this->input->post('upload_type',TRUE);
            if($upload_type == "audio")
            {
                $question_allowed_format = 'ogg|wav|mp3|mpeg|m4a';
            }
            $question_response_file_upload = $this->do_upload_file('image',$question_path,$question_allowed_format);

            if($question_response_file_upload['status'] == 'success')
            {
                $question_image = $question_response_file_upload['upload_data']['file_name'];
                $question_file_link = $question_path.$question_image;
            }
            else
            {
                $this->session->set_flashdata('error', $question_response_file_upload['error']); 
        ...
        {   
            $solution_path = "./assets/images/questions/solution";
            $solution_allowed_format = 'gif|jpg|png|bmp|jpeg';
            $solution_response_file_upload = $this->do_upload_file('solutionimage',$solution_path,$solution_allowed_format);

            if($solution_response_file_upload['status'] == 'success')
            {
                $solution_image = $solution_response_file_upload['upload_data']['file_name'];
                $solution_file_link = $solution_path.$solution_image;
            }
            else
            {
                $this->session->set_flashdata('error', $solution_response_file_upload['error']); 
    ...
    function update($quiz_id = NULL, $question_id = NULL) 
    {
        ...
        $nextques_data = $this->QuestionModel->get_nextques_by_id($question_id, $quiz_id);
        $prevques_data = $this->QuestionModel->get_prevques_by_id($question_id, $quiz_id);
        ...
        */
        ...
        $content_data = array('question_id' => $question_id, 
            'question_data' => $question_data, 
            'no_of_option' => $no_of_option, 
            'choices_array' => $choices_array, 
            'arr_is_correct' => $arr_is_correct, 
            'quiz_id' => $quiz_id,
            'question_type_is_match' => $question_type_is_match,  
            'nextques_data' => $nextques_data,
            'prevques_data' => $prevques_data
            );
        // load views
        $data['content'] = $this->load->view('admin/questions/form', $content_data, TRUE);
        $this->load->view($this->template, $data);
